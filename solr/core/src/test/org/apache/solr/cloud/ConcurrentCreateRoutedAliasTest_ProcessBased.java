...
/**
 * ProcessBased upgrade test for concurrent creation of time-routed aliases.
 *
 * <p>Transformed from {@link ConcurrentCreateRoutedAliasTest} to support checkpoint-based upgrade
 * testing with real Solr processes.
 */
@LuceneTestCase.AwaitsFix(bugUrl = "https://issues.apache.org/jira/browse/SOLR-12386")
public class ConcurrentCreateRoutedAliasTest_ProcessBased extends ProcessBasedUpgradeTestBase {
  ...
  @Test
  public void testConcurrentCreateRoutedAliasMinimal_NO_UPGRADE() throws Exception {
    upgradeCheckpoint = SolrUpgradeCheckpoints.NO_UPGRADE;
    testConcurrentCreateRoutedAliasMinimalImpl();
  }

  @Test
  public void testConcurrentCreateRoutedAliasMinimal_AFTER_CLUSTER_START() throws Exception {
    upgradeCheckpoint = SolrUpgradeCheckpoints.AFTER_CLUSTER_START;
    testConcurrentCreateRoutedAliasMinimalImpl();
  }

  private void testConcurrentCreateRoutedAliasMinimalImpl() throws Exception {
    // Initialize cluster with 4 nodes
    cluster =
    ...
    // (just
    // bothersome). Any use case creating large numbers of time routed aliases concurrently would
    // be
    // an EXTREMELY odd if not fundamentally broken use case. This test method is just here to
    // guard
    ...
    // intentional hard limit on the overseer queue, and I haven't tried to count the retries up
    // and
    // figure out if the requests are actually exceeding that limit or not, but the speed of
    // retries
    ...
    // users should never see an issue in the wild unless they are doing something we probably
    // don't
  ...
  @Test
  public void testConcurrentCreateRoutedAliasComplex_NO_UPGRADE() throws Exception {
    upgradeCheckpoint = SolrUpgradeCheckpoints.NO_UPGRADE;
    testConcurrentCreateRoutedAliasComplexImpl();
  }

  @Test
  public void testConcurrentCreateRoutedAliasComplex_AFTER_CLUSTER_START() throws Exception {
    upgradeCheckpoint = SolrUpgradeCheckpoints.AFTER_CLUSTER_START;
    testConcurrentCreateRoutedAliasComplexImpl();
  }

  private void testConcurrentCreateRoutedAliasComplexImpl() throws Exception {
    // Initialize cluster with 4 nodes
    cluster =
        new ProcessBasedMiniSolrCloudCluster.Builder()
            .withNodeCount(4)
